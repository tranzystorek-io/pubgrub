...
pub fn resolve<P: Package, R: RangeSet>(
    dependency_provider: &impl DependencyProvider<P, R>,
    package: P,
    version: impl Into<R::VERSION>,
) -> Result<SelectedDependencies<P, R::VERSION>, PubGrubError<P, R>> {
    let mut state = State::init(package.clone(), version.into());
    let mut added_dependencies: Map<P, Set<R::VERSION>> = Map::default();
            ...
            let dependencies =
                match dependency_provider
                    ...
                    Dependencies::Known(x) => {
                        ...
                        if let Some((dependent, _)) = x.iter().find(|(_, r)| r == &&R::none()) {
...
#[derive(Clone)]
pub enum Dependencies<P: Package, R: RangeSet> {
    /// Package dependencies are unavailable.
    Unknown,
    /// Container for all available package versions.
    Known(DependencyConstraints<P, R>),
}
...
pub type DependencyConstraints<P, R> = Map<P, R>;

/// Trait that allows the algorithm to retrieve available packages and their dependencies.
/// An implementor needs to be supplied to the [resolve] function.
pub trait DependencyProvider<P: Package, R: RangeSet> {
...
pub fn choose_package_with_fewest_versions<P: Package, R: RangeSet, T, U, I, F>(
...
impl<P: Package, R: RangeSet> OfflineDependencyProvider<P, R> {
    ...
    pub fn versions(&self, package: &P) -> Option<impl Iterator<Item = &R::VERSION>> {
    ...
    fn dependencies(
        ...
        package: &P,
        version: &R::VERSION,
    ) -> Option<DependencyConstraints<P, R>> {
