...
/// Base methods.
impl<R: RangeSet> Term<R> {
    /// A term that is always true.
    pub(crate) fn any() -> Self {
        Self::Negative(R::none())
    }

    /// A term that is never true.
    pub(crate) fn empty() -> Self {
        Self::Positive(R::none())
    }

    /// A positive term containing exactly that version.
    pub(crate) fn exact(version: R::VERSION) -> Self {
        Self::Positive(R::exact(version))
    }
    ...
    pub(crate) fn contains(&self, v: &R::VERSION) -> bool {
        match self {
    ...
    pub(crate) fn unwrap_positive(&self) -> &R {
        match self {
...
impl<R: RangeSet> AsRef<Term<R>> for Term<R> {
    fn as_ref(&self) -> &Self {
        &self
    }
}
...
pub mod tests {
    use super::*;
    use crate::{version::NumberVersion, range::Range};
