...
pub struct SatResolve<P: Package, R: RangeSet> {
    solver: varisat::Solver<'static>,
    all_versions_by_p: Map<P, Vec<(R::VERSION, varisat::Var)>>,
}

impl<P: Package, R: RangeSet> SatResolve<P, R> {
    pub fn new(dp: &OfflineDependencyProvider<P, R>) -> Self {
        let mut cnf = varisat::CnfFormula::new();

        let mut all_versions = vec![];
        let mut all_versions_by_p: Map<P, Vec<(R::VERSION, varisat::Var)>> = Map::default();
    ...
    pub fn sat_resolve(&mut self, name: &P, ver: &R::VERSION) -> bool {
    ...
    pub fn sat_is_valid_solution(&mut self, pids: &SelectedDependencies<P, R::VERSION>) -> bool {
